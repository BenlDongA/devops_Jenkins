...
const Cart = () => {
    ...
    const calculateTotal = (foodItems) => {
        return foodItems.reduce((total, item) => total + item.quantity * item.foodId.price, 0);
    };
 ...
 <OrderHeader>  
    ...
    />  
    <h2>Người đặt: {order.user && order.user.name ? order.user.name : 'Không xác định'}</h2>
                                ...
                                <FoodList>
                                    {order.food.map((item) => (
                                        <FoodItem key={item._id}>
                                            <p>
                                                - Món: {item.foodId.name} - SL: {item.quantity} - Giá: {item.foodId.price} VND
                                            </p>
                                        </FoodItem>
                                    ))}
                                </FoodList>
                                <TotalAmount>
                                    TỔNG TIỀN ĐƠN HÀNG: {calculateTotal(order.food)} VND
                                </TotalAmount>
                            </OrderInfo>
                        </OrderCard>
                    ))}
